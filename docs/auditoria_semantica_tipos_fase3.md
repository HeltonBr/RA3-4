...
| Potenciacao | Expoente deve ser `int`; por orientacao do professor, expoente literal `0` e aceito como inteiro positivo nesta linguagem. | `type_system.py`, `docs/regras_tipos_sequentes.md` |

## Decisao sobre INC-17

Com a orientacao do professor, o analisador aceita `^` com expoente literal inteiro `0`, tratando esse valor como inteiro positivo nesta linguagem. A regra estatica implementavel fica assim:
...
Quando o expoente vem de memoria ou expressao, o analisador verifica o tipo `int`. Essa decisao evita falso bloqueio para programas que dependem de valores definidos em tempo de execucao e preserva a convencao confirmada pelo professor de que `0` e inteiro positivo nesta linguagem.
