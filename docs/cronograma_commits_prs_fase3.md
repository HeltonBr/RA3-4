...
| 11/05/2026 | `feature/03-gramatica-bool-logicos` | Fechar gramatica LL(1) com bool e operadores logicos. | `feat: adiciona bool e operadores logicos a gramatica` | PR 03 - Gramatica aumentada | `docs/gramatica_atribuida.md`, FIRST/FOLLOW e tabela LL(1) atualizados. |
| 12/05/2026 | `feature/04-tabela-simbolos` | Fortalecer tabela de simbolos, definicoes, usos e reatribuicoes. | `feat: implementa tabela de simbolos semantica` | PR 04 - Tabela de simbolos | Uso antes da definicao e troca de tipo rejeitados. |
| 13/05/2026 | `feature/05-verificacao-tipos` | Ampliar verificacao de tipos para aritmetica, relacionais, logicos e controle. | `feat: valida regras de tipos da fase 3` | PR 05 - Sistema de tipos | Matriz de compatibilidade testada com validos e invalidos. |
| 14/05/2026 | `feature/06-arvore-atribuida` | Gerar arvore sintatica atribuida com tipo, status e referencias de simbolo. | `feat: gera arvore sintatica atribuida` | PR 06 - Arvore atribuida | JSON e Markdown da arvore atribuida conferidos. |
| 15/05/2026 | `feature/07-assembly-semantico` | Garantir que Assembly seja gerado apenas a partir de arvore atribuida valida. | `feat: bloqueia assembly em programas invalidos` | PR 07 - Assembly semantico | Assembly gerado para validos e bloqueado para invalidos. |
| 16/05/2026 | `feature/08-testes-obrigatorios` | Completar tres testes validos e suite de invalidos lexicos, sintaticos e semanticos. | `test: amplia cobertura obrigatoria da fase 3` | PR 08 - Testes obrigatorios | Todos os requisitos dos tres arquivos validos auditados. |
| 17/05/2026 | `feature/09-documentacao-defesa` | Fechar README, sequentes, decisoes do dossie, roteiro de defesa e checklist. | `docs: fecha documentacao formal da fase 3` | PR 09 - Documentacao final | README e docs sem referencias antigas conflitantes. |
| 18/05/2026 | `release/10-pre-auditoria` | Rodar suite completa, regenerar artefatos e preparar congelamento de features. | `chore: prepara pre-auditoria final da fase 3` | PR 10 - Pre-auditoria final | `python -m unittest discover -s tests -p "test_*.py" -v` aprovado e espelho atualizado. |
